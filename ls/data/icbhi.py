...
class ICBHIDataset(Dataset):
    ...
    #     self,
    #     balance_mode: str = "class",  # "class", "device", "both", or "none"
    #     alpha: float = 1.0,           # strength of device balancing (0–1)
    # ):
    #     """
    #     Compute per-sample weights dynamically based on chosen balancing mode.

    #     balance_mode:
    #         - "none":    uniform sampling
    #         - "class":   inverse label frequency
    #         - "device":  inverse device frequency
    #         - "both":    label × device weighting
    #     alpha:
    #         Controls how strongly device imbalance is corrected.
    #         alpha = 1.0 → full inverse frequency
    #         alpha < 1.0 → softened reweighting (sqrt, etc.)
    #     """
    #     if not self.train:
    #         return None
    ...
    #     if balance_mode in ["class", "both"]:
    #         if not self.multi_label:
    #             class_counts = np.bincount(
    #                 [s["label"] for s in self.samples],
    #                 minlength=self.dataset_cfg.n_cls
    #             )
    #             inv_freq = 1.0 / (class_counts + 1e-6)
    #             class_weights = np.array([inv_freq[s["label"]] for s in self.samples])
    #         else:
    #             labels_ml = np.array([s["multi_label"] for s in self.samples])
    #             label_freq = labels_ml.sum(axis=0).astype(float)
    #             inv_freq = 1.0 / (label_freq + 1e-6)
    ...
    #     if balance_mode in ["device", "both"]:
    #         if "device" in self.samples[0]:
    #             device_counts = {}
    #             for s in self.samples:
    #                 dev = s.get("device", "Unknown")
    #                 device_counts[dev] = device_counts.get(dev, 0) + 1
    #             inv_dev = {d: (1.0 / (c + 1e-6)) ** alpha for d, c in device_counts.items()}
    #             device_weights = np.array([inv_dev.get(s.get("device", "Unknown"), 1.0)
    #                                     for s in self.samples])
