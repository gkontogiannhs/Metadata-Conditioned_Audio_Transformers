...
#             # Load AudioSet checkpoint weights
#             sd = torch.load(audioset_ckpt_path, map_location=DEVICE)
...
#             # Load backbone weights (filter out mlp_head since dimensions differ)
#             model_dict = self.v.state_dict()
#             pretrained_dict = {}
            
#             for k, v in sd.items():
#                 # Remove 'v.' prefix if present
#                 if k.startswith('v.'):
#                     key = k[2:]  # Remove 'v.' prefix
#                 elif k.startswith('module.v.'):
#                     key = k[9:]  # Remove 'module.v.' prefix
#                 elif k.startswith('module.'):
#                     key = k[7:]  # Remove 'module.' prefix
#                 else:
#                     key = k
                
#                 # Skip mlp_head weights (wrong dimensions)
#                 if 'mlp_head' in key or 'head' in key:
#                     continue
...
#             if len(pretrained_dict) > 0:
#                 model_dict.update(pretrained_dict)
#                 self.v.load_state_dict(model_dict)
#                 if verbose:
#                     print(f"Loaded {len(pretrained_dict)} tensors from AudioSet checkpoint")
#             else:
#                 print("WARNING: No matching weights found in checkpoint! Training from ImageNet initialization.")
...
#             with torch.no_grad():
#                 # Original AudioSet grid: 12 (freq) × 101 (time) = 1212 patches
#                 orig_f_dim, orig_t_dim = 12, 101
                
#                 # Extract spatial tokens (skip CLS and distillation tokens)
...
#                 expected_patches = orig_f_dim * orig_t_dim
#                 if pos_embed.shape[1] != expected_patches:
#                     print(f"WARNING: Expected {expected_patches} patches, got {pos_embed.shape[1]}")
#                     # Adjust dimensions if needed
#                     orig_f_dim = int(pos_embed.shape[1] ** 0.5)
#                     orig_t_dim = pos_embed.shape[1] // orig_f_dim
#                     if verbose:
#                         print(f"Auto-detected grid: {orig_f_dim} x {orig_t_dim}")
...
#             with torch.no_grad():
#                 if hasattr(self.v.patch_embed.proj, 'weight'):
...
#         # ================================================================
#         #  ImageNet pretrained or random initialization
#         # ================================================================
#         else:
#             if model_size == 'tiny224':
#                 self.v = timm.create_model('vit_deit_tiny_distilled_patch16_224', pretrained=imagenet_pretrain)
